	...
	export default {
		data() {
			return {
				...
				tableHeader:[
					...
					{label:'旧合同金额',prop:'oldMoney'},
					{label:'新合同金额',prop:'newMoney'},
					{label:'变更描述',prop:'changeDetail'},
					{label:'流程名称',prop:'checkPathName'},
		...
		methods: {
			getListData() {
				...
				this.$u.api.changeContApi.getChangeContQueryPageList(this.fromValiData, this).then(res => {
			...
			handleProcess(params){
				uni.navigateTo({
				    url: '../contractVerity/process?params=' + encodeURIComponent(JSON.stringify(params))
				})
			},
			doSearch() {
